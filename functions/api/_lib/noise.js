...
export const SHOP_HOSTS = [
  ...
  // 🚛 เว็บซื้อขายรถ/รถมือสอง — ประกาศขายของผู้ใช้ ไม่ใช่ข่าว (เจ้าของส่งภาพมา 14 ก.ย. 2026:
  // "คุณยุ บจก.เอ็มซีพี เครน - Truck2Hand.com" — ชื่อบริษัทผู้ลงขายมีคำว่า ซีพี ซ่อนอยู่)
  // ⚠️ ใส่เฉพาะเว็บที่เป็น "ตลาดซื้อขาย" ล้วน — เว็บข่าวรถ (autospinn/headlightmag) มีข่าวจริง ห้ามใส่
  "truck2hand", "one2car", "taladrod", "rodkaidee", "chobrod", "carsome", "roddonjai",
];
...
/* 🏷 **หน้ารวมของ "ป้ายกำกับ/หมวดหมู่" ก็คือหน้ารวมบทความเหมือนกัน** (เจ้าของส่งภาพมา 11 ก.ย. 2026)
   เจอจริง: "ป้ายกำกับ: ซีพี - ไทยโพสต์" — หน้า tag ของเว็บ ไม่มีเนื้อข่าวเลย
   มีแต่ชื่อเครืออยู่ในพาดหัวเต็มๆ ด่านชื่อเครือจึงปล่อยผ่านทุกใบ ต้องดักที่รูปพาดหัว
   ⚠️ **ต้องอยู่ต้นพาดหัวและมีเครื่องหมาย : เสมอ** — คำว่า "ป้ายกำกับ" กลางประโยคเป็นข่าวจริงได้
      ("สั่งติดป้ายกำกับสินค้า…") ตัดเพลินจะกินข่าวจริง
   ⚠️ ดูที่ **พาดหัว** เท่านั้น เหมือน ARCHIVE_RE */
export const TAGPAGE_RE =
  /^\s*(?:ป้ายกำกับ|แท็ก|แท็ค|หมวดหมู่|tags?|category|categories|tagged)\s*[:：]/i;
...
export const CP_BRANDS = [
  ...
  // 🏦 กลุ่ม Ascend — เครือ CP (เจ้าของยืนยัน 8 ก.ย. 2026 หลังเจอข่าว "Ascend Bank" ถูกตัดทิ้ง)
  // ⚠️ **ใส่เป็นชื่อประกอบเท่านั้น ห้ามใส่ `ascend` เดี่ยว** — เป็นคำอังกฤษธรรมดา
  //    ("markets ascend" · "ascend the throne") บทเรียนเดิมกับ SLAPP → slapped
  //    · คำว่า `ascend` เดี่ยวไปอยู่ใน CP_WEAK_BRANDS แทน = ส่งให้ AI อ่านพาดหัวตัดสิน
  // ⚠️ ฝั่งไทยไม่ต้องกลัวชนคำอื่น — "แอสเซนด์" เป็นคำทับศัพท์ที่ไม่มีความหมายอื่น
  "ascend money", "ascend group", "ascend bank", "ascend commerce", "ascend travel",
  "แอสเซนด์", "ทรูมันนี่", "ทรูมั่นนี่", "truemoney", "true money",
  // 🎪 งานของเครือที่ขึ้นต้นด้วย CP — เจ้าของแจ้ง 8 ก.ย. 2026 (ข่าวงาน CP Innovation Expo
  // 3 ใบถูก AI ตัดทิ้งด้วย ai-no-cp) · เป็นชื่องานเฉพาะ ไม่ชนคำอื่น
  "cp innovation", "cp expo", "cp exposition",
];
...
export const CP_FALSE_RX = [
  ...
  // 💊 เบนโซไดอะ-ซีพี-น = ชื่อกลุ่มยา (benzodiazepine) — เจ้าของส่งภาพมา 9 ก.ย. 2026
  // ข่าว "รู้จัก 'Xanax' ยาคลายกังวลกลุ่มเบนโซไดอะซีพีน" หลุดเข้าคอลัมน์ CP
  // ⚠️ `ซีพี` ซ่อนอยู่กลางคำพอดี — กับดักเดียวกับ เอ็ม-ซีพี-ไอ (สารเคมี MCP) และ ซีพีพี (การแพทย์)
  "ไดอะซีพี",
  "benzodiazepin|diazepin",
  // 🚚 CP Foton = ผู้นำเข้ารถเพื่อการพาณิชย์แบรนด์ Foton — เจ้าของสั่งเอาออก 11 ก.ย. 2026
  // เจอจริง: "CP Foton จับมือ SPARK EV ลุยเพิ่มสถานีชาร์จกว่า 50 แห่ง"
  // ⚠️ ต้องจับ **ก่อน** ชั้น AI — ของเดิม `CP` เดี่ยวได้ weak แล้ว AI ตอบว่าใช่ (พลาดครั้งที่ 5)
  "cp\\s*foton",
  "ซีพี\\s*โฟตอน",
  // 🏗 เอ็มซีพี = ชื่อบริษัทคนละเจ้า (เจ้าของส่งภาพมา 14 ก.ย. 2026: "บจก.เอ็มซีพี เครน")
  // ⚠️ **ด่าน weak จับไม่ได้** เพราะมันดูว่า "ซีพี ถูกอักษรไทยขนาบทั้ง 2 ข้าง" แต่ใบนี้
  //    ข้างหลังเป็นช่องว่าง ("เอ็มซีพี เครน") → ได้ strong = ผ่านฟรี ไม่เคยไปถึงชั้น AI เลย
  //    กับดักเดียวกับ เอ็ม-ซีพี-ไอ (สารเคมี) ที่เคยจดไว้ แต่คราวนั้นมีขีดคั่นจึงโดนด่าน weak
  "เอ็มซีพี",
  // ⚽ ทรู แบงค็อก ยูไนเต็ด = สโมสรฟุตบอล — ข่าวผลบอล/โปรแกรมแข่ง ไม่ใช่ข่าวของเครือ
  // (เจ้าของสั่งเอาออก 14 ก.ย. 2026) · ตระกูลเดียวกับ "ทรูดิจิทัล พาร์ค" — ชื่อมีคำว่า ทรู
  // แต่ข่าวที่พูดถึงมันเป็นเรื่องอื่น · ⚠️ ตัดแค่ชื่อสโมสร ไม่ได้ตัดข่าวกีฬาทั้งหมด
  "ทรู\\s*แบงค็อก",
  "true\\s*bangkok",
...
export function noiseReason(it, title, src) {
  ...
  if (TAGPAGE_RE.test(title)) return "archive-page"; // หน้ารวมป้ายกำกับ/หมวดหมู่ — เรื่องเดียวกัน
...
// ⚠️ คำที่ "อาจ" เป็นชื่อเครือ แต่เป็นคำธรรมดาได้ด้วย → ไม่ให้ผ่านฟรี ส่งให้ AI อ่านพาดหัวตัดสิน
// · `cp`     — เป็นตัวย่อของอย่างอื่นเยอะ (cerebral palsy · ภาควิชาวิศวะคอมฯ · รหัสรุ่นสินค้า)
// · `ascend` — เป็นคำกริยาอังกฤษ ("markets ascend") · ชื่อเต็มอย่าง "Ascend Money" อยู่ใน
//              CP_BRANDS แล้ว จึงผ่านตั้งแต่ด่านแรก ตัวนี้รับเฉพาะกรณีที่พาดหัวเขียน Ascend เดี่ยว
const CP_WEAK_BRANDS = ["cp", "ascend"];
 ...
 */
/* 🏢 **`CP` ที่ทำตัวเป็น "ประธานของประโยค" = องค์กร ไม่ใช่ตัวย่ออย่างอื่น**
 *
 * เจ้าของเจอจริง 8 ก.ย. 2026: ข่าวงาน CP Innovation Expo 3 ใบถูกตัดด้วย `ai-no-cp`
 * ใบที่ยากสุดคือ «นวัตกรรมแบบไหนที่ "ไปต่อ" … ?...**CP เปิด** "Behind the Judging Room"»
 * — มีแค่ `CP` เดี่ยว ไม่มีคำบริบทเดิมเลย (ไม่ใช่ไฮสปีด/แอร์พอร์ตลิงก์)
 *
 * สัญญาณที่ใช้: **`CP` ตามด้วยคำกริยาที่ประธานต้องเป็นองค์กร** (เปิด · เผย · ผนึก · จับมือ …)
 * `cerebral palsy` หรือ `ภาควิชา CP` ไม่มีทางตามด้วยคำพวกนี้
 *
 * ⚠️ ต้องเป็น **คำที่ติดกับ CP** (เว้นวรรคได้ไม่เกิน 2 ตัว) ไม่ใช่โผล่ที่ไหนก็ได้ในพาดหัว
 *    ไม่งั้น "เปิด" ที่อยู่ต้นประโยคจะทำให้ CP อะไรก็ตามกลายเป็นเครือหมด
 */
/* #️⃣ **`CP` ตัวละตินที่ติดกับอักษรไทยโดยไม่มีช่องว่าง = แฮชแท็ก/ชื่อแคมเปญของเครือ**
 *
 * เจ้าของส่งภาพมา 9 ก.ย. 2026: **"CPทุกคำใส่ใจFilmAwards2026 - เข้มข้น กับคนข่าวเกษตรตัวจริง"**
 * ถูกตัดด้วย `ai-weak-cp` (AI อ่านแล้วบอกว่าไม่ใช่ข่าวของเครือ) ทั้งที่เป็นชื่อโครงการของเครือ
 *
 * ⚠️ **ต่างจากกับดัก "ชื่อเครือกลางคำอื่น" คนละเรื่อง** — อันนั้นคือ `ซีพี` **ตัวไทย** ที่ถูก
 *    อักษรไทยขนาบ (เอ็ม-ซีพี-ไอ · เบนโซไดอะ-ซีพี-น) · อันนี้คือ `CP` **ตัวละติน** แล้วต่อด้วยไทย
 *    ซึ่งภาษาไทยไม่เขียนแบบนี้ตามธรรมชาติ — เป็นรูปแบบของแฮชแท็ก/ชื่อแคมเปญเท่านั้น
 *
 * 🚫 **ต้องไม่มีอักษรละติน/ตัวเลขนำหน้า** ไม่งั้น `MCPไทย` จะกลายเป็นเครือ
 * 🚫 **ต้องติดกันจริงๆ ห้ามมีช่องว่าง** — "CP ทุกคำ" ยังเป็น weak ตามเดิม (ให้ AI ตัดสิน)
 */
const CP_THAI_GLUE_RE = /(?<![A-Za-z0-9])cp(?=[ก-๛])/i;

const CP_ORG_VERB =
  /\bcp\b[\s"'’”:：\-–]{0,2}(?:เปิด|เผย|ผนึก|จับมือ|ลงทุน|ทุ่ม|คว้า|ชู|รุก|ส่ง|ปั้น|เดินหน้า|ประกาศ|แถลง|ตั้งเป้า|ยัน|ย้ำ)/i;

const CP_STRONG_CTX_RE =
  /ไฮสปีด|ไฮ-?สปีด|high\s*speed|แอร์พอร์ต\s*(?:เรล)?\s*ลิงก์|airport\s*rail|รฟท|สามสนามบิน|3\s*สนามบิน|เครือ|กลุ่มซีพี/i;
const CP_STRONG_CTX = {
  test: (t) => CP_STRONG_CTX_RE.test(t) || CP_ORG_VERB.test(t) || CP_THAI_GLUE_RE.test(t),
};
