...
// pol/news/sport ล้วนมีคำเฉพาะของตัวเองชัดเจน ถ้าไม่มีคำพวกนั้นเลย = ไม่มีหลักฐาน
// (เจอจริง: "PREMIUM" ถูกจัดเป็นกีฬา น่าจะสับสนกับ "Premier League")
// ส่วน biz/ent เป็นชื่อแบรนด์กับชื่อคน ไม่มีคำบอก จึงตรวจแบบนี้ไม่ได้ ปล่อยตาม AI
const NEED_EVIDENCE = ["pol", "news", "sport"];

function vetoCat(name, cat) {
  if (NEED_EVIDENCE.includes(cat) && !CAT_RE_MAP[cat].test(name)) return "ent";
  return cat;
}
