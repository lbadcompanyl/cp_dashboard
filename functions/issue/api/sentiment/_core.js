 ...
 * ============================================================ */
/* ══════════════════════════════════════════════════════════════════════
 * profile `cp_blackchin` — ประเด็นปลาหมอคางดำโดยเฉพาะ
 *
 * เจ้าของสั่งแยก 4 ก.ย. 2026: "แยกดีกว่า เพราะเผาข้าวโพด ถ้าไม่ได้เอ่ยถึง
 * ตัวบริษัทโดยตรงก็อาจหมายถึงชาวสวน"
 *
 * 🎯 ต่างจาก `cp_comment` ข้อเดียวแต่สำคัญมาก — **ประเด็นนี้ CP ถูกชี้ว่าเป็นต้นตอโดยตรง**
 *    (เป็นผู้นำเข้าปลา) คอมเมนต์ที่ทำให้วิกฤตดูเบาลง/หนักขึ้น จึงสะท้อนถึง CP
 *    แม้ไม่เอ่ยชื่อ — ซึ่งประเด็นอื่น (ฝุ่น · เผาข้าวโพด) ใช้กฎนี้ไม่ได้
 *    เพราะคนโดนด่าอาจเป็นชาวไร่หรือรัฐ ไม่ใช่ CP
 *
 * 📊 มาจากข้อมูลจริง: เจ้าของแก้ป้ายไว้ 202 ใบ · 116 ใบเป็นแกน CP
 *    และ **ไม่มีใบไหนเอ่ยชื่อ CP เลยสักใบ** → ยืนยันว่าใช้นิยามนี้จริง
 *    ใบที่แก้เป็นบวก 88 ใบ: กินได้/อร่อย 45 · อีสานจัดการได้ 38
 *    ใบที่แก้เป็นลบ  28 ใบ: ระบบนิเวศพัง 15 · กระทบบ่อกุ้ง 6
 *
 * ⚠️ **เขียนแยกเป็นข้อความของตัวเอง ไม่ได้ใช้ร่วมกับ systemTwoLens()**
 *    ยอมให้ซ้ำ เพราะถ้าไปแก้ของกลาง prompt ของ `cp_comment` จะขยับ
 *    แล้วตัวเลข 92.8% ที่วัดไว้ใช้อ้างอิงไม่ได้อีก (มี sha256 ล็อกไว้ใน profiles.mjs)
 *
 * 🔴 **ยังไม่เคยวัดความแม่นเลยสักครั้ง** — ยังไม่มีชุดสอบของประเด็นนี้
 *    ห้ามพูดว่าแม่นกว่า/แย่กว่า `cp_comment` จนกว่าจะวัดจริง
 * ══════════════════════════════════════════════════════════════════════ */
const BLACKCHIN_SHOTS = [
  /* ที่มา: syn = คัดจากชุดสอน 2,000 ใบที่เจ้าของส่งมา (ตัด 157 ใบที่อีโมจิขัดกับป้ายทิ้งแล้ว)
           real = คอมเมนต์จริงจากกองรอตรวจ — เจ้าของแก้ป้ายเอง
     🚫 ใบที่อยู่ตรงนี้ห้ามเอาไปอยู่ในชุดสอบ = เอาเฉลยมาสอนตัวเอง (leakcheck.py คอยจับ) */
  { t: "โรงงานช่วยรับซื้อ ชาวบ้านมีที่ขายดีขึ้น", cp: "Positive", src: "syn" },
  { t: "เอามาทำอาหารก็ดี แต่อย่าให้แพงมาก", cp: "Positive", src: "syn" },
  { t: "ทำขายก็ดี ขอแค่บอกให้รู้ว่าปลาอะไร สิ 🙏", cp: "Positive", src: "syn" },
  { t: "ทำน้ำปลากับปลาร้าอร่อยดีนะ ได้ใช้ประโยชน์", cp: "Positive", src: "syn" },
  { t: "คือ เอามาทำอาหารก็ดี แต่อย่าให้แพงมาก อ่ะ", cp: "Positive", src: "syn" },
  { t: "อร่อยดีนะ แต่ราคาต้องถูกกว่านี้ สิ 👍", cp: "Positive", src: "syn" },
  { t: "ตอนนี้รับซื้อราคาเท่าไรแล้ว ครับ", cp: "Neutral", src: "syn" },
  { t: "เอาจริง ตอนนี้ระบาดไปกี่จังหวัดแล้ว ค่ะ เฮ้อ", cp: "Neutral", src: "syn" },
  { t: "ปลาหมอคางดำเป็น GMO หรือเปล่า ว่ะ 555", cp: "Neutral", src: "syn" },
  { t: "สรุป ใครรู้บ้างว่าใครถือหุ้นใหญ่ สิ ...", cp: "Neutral", src: "syn" },
  { t: "รัฐบาลไม่ทำอะไรเลย ปล่อยปละมานาน ...", cp: "Neutral", src: "syn" },
  { t: "คือ แล้วใครจะเป็นคนชดเชยเกษตรกร อ่ะ 555", cp: "Neutral", src: "syn" },
  { t: "ส่วนตัว ดีจังโรงงานได้วัตถุดิบฟรีๆจากวิกฤต 🫡", cp: "Negative", src: "syn" },
  { t: "ขอบคุณต้นตอที่มอบปลาหมอคางดำให้ทั้งประเทศ 🫡", cp: "Negative", src: "syn" },
  { t: "เอาจริง เยี่ยมเลยได้ค่าแรงคุ้มสุดๆ 555 สินะ", cp: "Negative", src: "syn" },
  { t: "ส่วนตัว ผลงานรัฐเยี่ยมมาก ปลาเพิ่มขึ้นทุกปี 🫡", cp: "Negative", src: "syn" },
  { t: "ฟอกขาวได้เนียนมาก ชื่นชมเลย 🤣", cp: "Negative", src: "syn" },
  { t: "สรุป ดีจังกินแล้วได้สารแถมฟรี ??", cp: "Negative", src: "syn" },
  { t: "ผมเคยกินครับ เอามาทอดอร่อยกว่าปลานิลครับ จริงๆครับ", cp: "Positive", src: "real" },
  { t: "กินได้ครับ อร่อยด้วย", cp: "Positive", src: "real" },
  { t: "ทอดกรอบๆ กับแกล้มชั้นเลิศเลย", cp: "Positive", src: "real" },
  { t: "มาเลยครับอีสานรออยู่นะ ปลาร้ารออยู่ 🤣", cp: "Positive", src: "real" },
  { t: "หลุดมาอีสานมึงสูญพันธุ์แน่🤣", cp: "Positive", src: "real" },
  { t: "แต่ถ้าปลาหมอคางดำโต กุ้ง ปู ไม่เหลือ 😂", cp: "Negative", src: "real" },
  { t: "กว่ากุ้งจะโตคางดำแดกเรียบคับ", cp: "Negative", src: "real" },
  { t: "ระบบนิเวศพัง ไม่คุ้มหรอก", cp: "Negative", src: "real" },
  { t: "มีทุกที่ ที่มีน้ำ คางดำ ปลาแต่ง สุดยอดแห่งลุ่มน้ำ", cp: "Negative", src: "real" },
  { t: "😂", cp: "Neutral", src: "real" },
  { t: "ใครเอามันมา", cp: "Neutral", src: "real" },
  { t: "มีแต่คลิปเก่าๆๆ", cp: "Neutral", src: "real" },
];

/** prompt ของ profile `cp_blackchin` — โครงเดียวกับ systemTwoLens() แต่กฎแกน CP ต่างกัน */
function systemBlackchin() {
  const ex = BLACKCHIN_SHOTS.map(e => `"${e.t}"\n→ {"cp":"${e.cp}"}`).join("\n");
  return [
    "คุณคือระบบวิเคราะห์ sentiment คอมเมนต์ภาษาไทย ใต้ข่าว **ปลาหมอคางดำ**",
    "ข่าวประเด็นนี้ เครือ CP ถูกชี้ว่าเป็นผู้นำเข้าปลาชนิดนี้เข้าประเทศ",
    "วิเคราะห์ทุกคอมเมนต์ตาม 2 แกนพร้อมกัน ตอบเป็น JSON เท่านั้น ห้ามมีข้อความอื่น",
    "",
    "═══ แกน 1: cp (ผลต่อเครือ CP ในประเด็นนี้) ═══",
    "⚠️ **ประเด็นนี้ CP คือผู้ถูกกล่าวหา** คอมเมนต์จึงสะท้อนถึง CP ได้แม้ไม่เอ่ยชื่อ",
    "",
    "• Negative = ทำให้วิกฤตดูหนักขึ้น หรือชี้ตัวคนทำ",
    '  ระบบนิเวศพัง · ปลาพื้นถิ่นหมด · กระทบบ่อกุ้ง/เกษตรกร · แพร่พันธุ์คุมไม่อยู่',
    '  · ด่าผู้นำเข้า/ต้นตอ/นายทุน/เจ้าสัว · ประชดว่า "ขอบคุณคนเอามาปล่อย"',
    "",
    "• Positive = ทำให้วิกฤตดูเบาลง หรือแก้ต่างให้",
    '  บอกว่ากินได้/อร่อย/เอาไปทำอาหารได้ · บอกว่าคนจะจับกินจนหมดเอง',
    '  · บอกว่าไม่น่ากลัวอย่างที่คิด/ข่าวเกินจริง · ชมสิ่งที่เครือทำเพื่อแก้ปัญหา',
    '  · ตำหนิคนที่มาด่าโดยยังไม่ดูให้จบ',
    "  🔴 **ข้อนี้ต่างจาก profile ทั่วไป** — ที่นั่น \"ปลานี่กินได้นะ อร่อย\" เป็น Neutral",
    "     แต่ในประเด็นนี้ การบอกว่ากินได้ = ลดน้ำหนักวิกฤตที่ CP ถูกกล่าวหา → Positive",
    "",
    "• Neutral = ถามข้อมูล · เล่าข้อเท็จจริงเฉยๆ · ด่ารัฐล้วนๆ โดยไม่แตะนายทุน",
    '  · อีโมจิ/เสียงหัวเราะล้วน · คุยเรื่องอื่นที่ไม่เกี่ยวกับวิกฤต',
    "  ⚠️ ด่ารัฐ ≠ ด่า CP — แต่ถ้าประโยคบอกว่า **รัฐเอื้อ/อุ้ม/ไม่กล้าแตะนายทุน** = Negative",
    "",
    "═══ แกน 2: oc (อารมณ์รวมของคอมเมนต์) ═══",
    "โกรธ/ไม่พอใจ/ตำหนิ = Negative · ชอบ/สนุก/เห็นด้วย = Positive · เฉย/ถาม = Neutral",
    "⚠️ แกนนี้ดูที่ **น้ำเสียง** ไม่เกี่ยวกับว่าใครได้ประโยชน์",
    '   ("มาเลยอีสานรออยู่ 🤣" = สนุก → oc Positive · และ cp Positive เพราะลดน้ำหนักวิกฤต)',
    "",
    "═══ ประชด (s) ═══",
    "s=1 เมื่อคำที่ใช้ตรงข้ามกับความหมายจริง · **ตีตามความหมายจริง ไม่ใช่ตามคำ**",
    "",
    "═══ ตัวอย่าง (แกน cp) ═══",
    ex,
    "",
    'ตอบเป็น array เท่านั้น: [{"i":1,"cp":"...","oc":"...","s":0}, ...]',
    "⚠️ ต้องตอบครบทุกข้อ เรียงตามลำดับที่ให้มา ห้ามข้าม ห้ามสลับ",
  ].join("\n");
}
...
const PROFILES = {
  ...
  /* 🐟 ประเด็นปลาหมอคางดำ — CP ถูกชี้ว่าเป็นต้นตอโดยตรง กฎแกน CP จึงต่างจากตัวทั่วไป
     🔴 ยังไม่เคยวัดความแม่น — ยังไม่มีชุดสอบของประเด็นนี้ */
  cp_blackchin: {
    rubric_version: "blackchin-v1",
    input: "comment",
    system: () => systemBlackchin(),
    shots: () => BLACKCHIN_SHOTS,
    lenses: ["cp", "overall"],
    default_lens: "cp",
    doc: "RUBRIC-BLACKCHIN.md",
    measured: false,          // ⚠️ ห้ามเขียนเป็น true จนกว่าจะวัดจริงแล้วจดลง BASELINE.md
  },
 ...
 */
/** @param profileId ชื่อ profile — ไม่ส่งมา = `cp_comment` (พฤติกรรมเดิมทุกอย่าง)
 *  ⚠️ ห้ามเปลี่ยนค่าปริยาย ไม่งั้น prompt ของ cp_comment ขยับ แล้ว 92.8% ใช้อ้างไม่ได้ */
async function classifyTwoLens(texts, env, acc, context, effort, profileId) {
  ...
  /* prompt มาจาก profile · ชื่อที่ไม่รู้จักต้องพัง ไม่ใช่เงียบๆ ใช้ตัวปริยาย
     (ถ้าตกกลับเงียบ จะได้ผลจาก rubric คนละตัวโดยไม่มีใครรู้) */
  const prof = getProfile(profileId);
  if (!prof) throw new Error(`ไม่รู้จัก profile "${profileId}"`);
...
async function analyze(opts, env) {
  ...
  /* 🐟 เลือกชุดเกณฑ์ (profile) — หน้าเว็บส่ง `profile` มา · ไม่ส่ง = ตัวปริยาย
     ⚠️ ชื่อที่ไม่รู้จักต้องพังตั้งแต่ตรงนี้ ไม่ใช่ไปเงียบๆ ใช้เกณฑ์คนละตัว */
  const profileId = opts.profile || DEFAULT_PROFILE;
  if (!getProfile(profileId)) throw new Error(`ไม่รู้จัก profile "${profileId}" — มีให้เลือก: ${Object.keys(PROFILES).join(", ")}`);
  ...
  for (let i = 0; i < askIdx.length; i += CHUNK) {
    ...
    const part = await classifyTwoLens(batch, env, tokens, opts.post_context || collected.post_title || "", undefined, profileId);
    asked.push(...part);
  ...
  return {
    ...
    profile: profileId,
    rubric_version: PROFILES[profileId].rubric_version,
    /* 🔴 ยังไม่เคยวัดความแม่นของ profile นี้หรือยัง — หน้าเว็บต้องขึ้นป้ายเตือน
       ไม่งั้นคนอ่านตัวเลขแล้วนึกว่าเชื่อได้เท่ากับตัวที่วัดมาแล้ว 92.8% */
