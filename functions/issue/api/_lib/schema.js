/**
 * schema.js — โครงตาราง D1 ของคอลัมน์ "คนพูดถึงเรา"
 * ==================================================
 * 🎯 **ที่นี่คือความจริงกลางที่เดียว** — `tools/zocial/schema.sql` เป็นไฟล์ที่ "สร้างจากที่นี่"
 *    ไว้ให้คนอ่าน · สร้างใหม่ด้วย `node tools/zocial/gen-schema.mjs`
 *    เทสต์ `tests/zocial.mjs` เทียบ 2 ไฟล์ให้ ถ้าไม่ตรงกันจะตก (กันของจริงกับเอกสารแตกสายกัน)
 *
 * ⚠️ Pages Functions โหลดไฟล์ .sql ไม่ได้ (ไม่มีระบบอ่านไฟล์) จึงต้องเป็น .js
 * 🚫 ห้ามใส่ DROP / ALTER ที่ทำลายข้อมูลลงในลิสต์นี้ — มันถูกรันทุกครั้งที่ isolate เกิดใหม่
 */
...
`INSERT OR IGNORE INTO setting (key, value) VALUES ('retention_days','${RETENTION_DEFAULT_DAYS}')`,
];

/** ตารางที่ตัวลบ retention แตะได้ — 🚫 daily_aggregate ห้ามอยู่ในลิสต์นี้เด็ดขาด */
export const PURGE_TABLES = ["raw_messages", "daily_news"];
