...
(function () {
  ...
  const Flags = {
    init(opts = {}) {
      ...
      //
      // ⚠️ **ห้ามให้คำขอนี้แซงคำขอของคอลัมน์แรก** (เจ้าของสั่ง 25 ส.ค. 2026)
      //    ค่าที่ได้จากที่นี่มีไว้ระบายสถานะปุ่ม ⚑/🗂 เท่านั้น ไม่ได้วาดเนื้อหาอะไรเลย
      //    แต่ของเดิมยิงตอน init() ซึ่งเกิดก่อน load() — วัดจริงบนมือถือ: flags เริ่มที่
      //    44 ms ส่วนข่าวของคอลัมน์แรกเริ่มที่ 99 ms = ของที่ผู้ใช้รออยู่ต่อคิวอยู่หลัง
      //    ของที่ไม่มีใครรอ · หน้าที่ส่ง opts.after มาจะรอให้คำขอนั้นออกตัวไปก่อน
      //    · ไม่ส่งมา (เช่น /admin/ ที่ flag คือเนื้อหาของหน้า) = ยิงทันทีเหมือนเดิม
      if (opts.after && typeof opts.after.then === "function") {
        opts.after.then(syncPull, syncPull); // ล้มเหลวก็ต้องยิง ไม่งั้นปุ่มค้างไม่มีสถานะ
      } else {
        syncPull();
      }
