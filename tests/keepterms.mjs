...
for (const [tag, file] of [["trend", "../functions/api/trend/feeds.js"],
  ...
  ok("ส่งคำที่ Google ไฮไลต์ไว้ไปด้วยเมื่อคอลัมน์ไม่มีลิสต์ของตัวเอง",
    /bodyHasKeep\(cache, items\[i\]\.link, extra\.length \? extra : verdict\[i\]\.terms\)/.test(src));
  ok("ไม่เหลือของเดิมที่ส่ง extra ไปดื้อๆ",
    !/bodyHasKeep\(cache, items\[i\]\.link, extra\)\)/.test(src));
}
