...
// ── [7] ว่าง 3 แบบ ─────────────────────────────────────────────────────
//   🚫 "คำค้น" ไม่ใช่ "ตัวกรอง" — พิมพ์คำแล้วไม่เจอ ห้ามบอกให้ "ลดตัวกรอง"
//      เพราะยังไม่ได้ตั้งตัวกรองอะไรเลย ไม่มีอะไรให้ลด (แก้ 17 ก.ย. 2026)
//      รายละเอียดเต็ม + ด่านของทั้ง 2 หน้าอยู่ที่ `archivetabs.mjs` [6]
console.log("\n[7] สถานะว่าง");
{
  ...
  ok("ค้นแล้วไม่เจอ = บอกให้ชัด พร้อมคำที่ค้น",
     /ไม่มีข่าวที่ตรงกับที่ค้น/.test(txt) && txt.includes("ไม่มีทางมีคำนี้อยู่จริงหรอกนะจ๊ะ"), txt.slice(0, 100));
  ok("🚫 ไม่บอกให้ลดตัวกรอง ทั้งที่ไม่ได้ตั้งตัวกรองไว้", !/ลดตัวกรอง|กรองไว้/.test(txt), txt.slice(0, 100));
  ok("ค้นแล้วไม่เจอ = มีปุ่มล้างคำค้นให้กด", !!(await p.$("#list [data-clearq]")));
  await p.click("#list [data-clearq]");
