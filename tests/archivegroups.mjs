...
const CASES = [
  ["ศาลอุทธรณ์ยืนตามชั้นต้น รับฟ้องแบบกลุ่ม เรียกค่าเสียหายปลาหมอคางดำ", "ศาล/คดีความ"],
  ["รมว.เกษตรฯ สั่งการด่วน ตั้งคณะทำงานแก้ปัญหาปลาหมอคางดำ", "ท่าทีภาครัฐ"],
  ["เคาะงบกลาง 350 ล้าน ลุย 7 มาตรการเร่งตัดวงจรปลาหมอคางดำ", "มาตรการรัฐ+CPF"],
  ["ผลตรวจ DNA ปลาหมอคางดำ กรมประมงเผยแหล่งที่มา", "งานวิจัย DNA"],
  ["ชาวบ้านแปรรูปปลาหมอคางดำเป็นลูกชิ้น สร้างรายได้เสริม", "ใช้ประโยชน์"],
  ["เปิดปม 11 บริษัทส่งออกปลาสวยงาม ปัญหาตรวจสอบย้อนกลับ", "ปัญหาลักลอบ / เพาะพันธุ์สัตว์ต่างถิ่น"],
  ["ส่องเอเลี่ยนสปีชีส์ในไทย ปลาซัคเกอร์ หอยเชอรี่ ผักตบชวา", "การระบาดของสัตว์ต่างถิ่น"],
  ["รู้จักปลาหมอคางดำ กินพืชเป็นหลัก อมไข่ในปาก ทนเค็มได้ดี", "สรีระ/พฤติกรรม"],
  ["เปิดภาพมุมสูงบรรยากาศริมเขื่อนยามเช้า นักท่องเที่ยวคึกคัก", "อื่น ๆ"],
  // 🛡 หมวดที่ 10 (เจ้าของเพิ่ม 18 ก.ย. 2026) — อยู่ **หลัง** "อื่น ๆ" ในไฟล์ config
  //    แต่ต้องโผล่ **ก่อน** "อื่น ๆ" บนแถบแท็บ (ดู [1] "แท็บสุดท้ายคือถังรับ")
  ["ข้อเท็จจริงคลาดเคลื่อน ยืนยันไม่ใช่ต้นเหตุ ยังไม่พบหลักฐานความเสียหาย", "ข้อหักล้างความเสียหาย"],
];
...
console.log("\n[1] จัดหมวดตามคำในพาดหัว");
{
  ...
  ok("มีแท็บ 'ทั้งหมด' + ครบ 10 หมวด", ts.length === 11 && ts[0].id === "", String(ts.length));
...
console.log("\n[7] ด่านระดับโค้ด");
{
  ...
  ok("ไฟล์หมวดมีครบ 10 หมวด", (CFG.match(/^\s{2}\{\s*$|^\s{2}\{ id:/gm) || []).length === 10,
     String((CFG.match(/^\s{2}\{\s*$|^\s{2}\{ id:/gm) || []).length));
  // 🎯 ถังรับต้องหาจากธง `etc: true` **ไม่ใช่ "ตัวสุดท้ายของลิสต์"** อีกต่อไป
  //    (ลำดับในไฟล์ = เลขในชีต และเจ้าของวาง "อื่น ๆ" ไว้ที่ 9 ไม่ใช่ 10)
  //    เผลอกลับไปใช้ `TOPICS.length - 1` เมื่อไหร่ ข่าวที่เดาไม่ออกจะไปกองที่หมวดหักล้างแทน
  ok("ถังรับหาจากธง etc: true และไม่มีคำของตัวเอง",
     /id: "etc"[^}]*any: \[\][^}]*etc: true/.test(CFG), "ต้องมี any ว่าง + etc: true");
  ok("🚫 app.js ไม่ได้เดาถังรับจากตัวสุดท้ายของลิสต์",
     /findIndex\(\(t\) => t\.etc\)/.test(APP) && !/const ETC = TOPICS \? TOPICS\.length - 1/.test(APP));
  ok("ลำดับบนแถบแท็บดันถังรับไปท้ายเสมอ",
     /function tabOrder\(\)/.test(APP) && /filter\(\(i\) => !TOPICS\[i\]\.etc\)\.concat\(/.test(APP));
...
console.log("\n[8] ชีตติ๊กหมวดมาแล้ว — ต้องยึดชีตก่อนเดา");
{
  ...
  // พาดหัวทุกใบมีคำของหมวด "ศาล/คดีความ" (w:3 ชนะทุกหมวด) — ถ้าโค้ดเผลอเดาทับ
  // ทุกใบจะไปกองที่หมวดแรกหมด เทสต์จะจับได้ทันที
  //    ⚠️ **พาดหัวต้องต่างกันจริง** ไม่งั้นโดนตัวรวมข่าวซ้ำ (groupStories) ยุบเป็นการ์ดเดียว
  //       แล้วป้ายหมวดของใบที่ถูกยุบจะหายไปจากหน้าจอ — เจอตอนรันจริง ตกไป 1 ข้อ
  const SHEET = [
  ...
  ok("ป้ายบนการ์ดตรงกับหมวดที่ติ๊กไว้", tags.includes("มาตรการรัฐ+CPF") && tags.includes("งานวิจัย DNA"),
