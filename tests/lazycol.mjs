...
console.log("\n[1] มือถือ — เปิดหน้ามาต้องยิงแค่คำขอเดียว");
{
  ...
  // ⚠️ คอลัมน์แรกของ /trend/ คือ alert2 (เจ้าของสลับลำดับ 11 ก.ย. 2026)
  //    สลับลำดับใน index.html เมื่อไหร่ ต้องมาแก้ชื่อคอลัมน์ 2 ข้อล่างนี้ด้วย
  ok("คอลัมน์แรกวาดข่าวครบ", c.alert2 === 20, JSON.stringify(c.alert2));
  ok("คอลัมน์ที่มี endpoint ของตัวเองยังไม่ถูกดึง", c.xtrends === "รอ" && c.trends === "รอ", JSON.stringify(c));
  // ⚠️ ข้อนี้วัด "ประหยัดแรงวาด" — news มีข้อมูลอยู่ในมือแล้ว (มากับคำขอเดียวกับคอลัมน์แรก)
  ...
  ok("คอลัมน์ที่ยังปัดไปไม่ถึงต้องไม่ถูกวาด แม้ข้อมูลจะมาถึงแล้ว", c.news === "รอ", JSON.stringify(c));
