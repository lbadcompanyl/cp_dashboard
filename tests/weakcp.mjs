...
// 🔴 อัปเดต 14 ก.ย. 2026 — เคส "เอ็มซีพี…" **เข้มขึ้นจาก weak → ตัดทิ้งเลย**
//    เจ้าของเจอ "บจก.เอ็มซีพี เครน" ที่ได้ `strong` ผ่านฟรี (ข้างหลังเป็นช่องว่าง
//    ด่าน weak จึงมองไม่เห็น) → ใส่ `เอ็มซีพี` ลง CP_FALSE_RX ตรงๆ
//    ผลพลอยได้: เอ็มซีพีไอ/เอ็มซีพีดี (สารเคมี) เลิกไปกวน AI แล้ว ตัดตั้งแต่ด่านแรก
const CUT = [
  ...
  ["คุณยุ บจก.เอ็มซีพี เครน - Truck2Hand.com", "เคสจริง 14 ก.ย. — เคยได้ strong ผ่านฟรี"],
];
for (const [t, why] of CUT) ok(`ตัดทิ้งเลย: ${why}`, cpEvidence(t) === "", cpEvidence(t));

// ⚠️ ยังต้องมีเคสที่พิสูจน์ว่า **กลไก weak เองยังทำงาน** ไม่งั้นลบกลไกทิ้งก็ยังผ่าน
// (ชื่อเครือถูกอักษรไทยขนาบทั้ง 2 ข้าง และไม่ได้อยู่ในลิสต์ชื่อลวง)
ok("weak: ซีพี ถูกอักษรไทยขนาบทั้ง 2 ข้าง ยังต้องส่งให้ AI",
  cpEvidence("บริษัทเอสซีพีเค จำกัด แจ้งผล") === "weak",
  cpEvidence("บริษัทเอสซีพีเค จำกัด แจ้งผล"));
...
console.log("\n[3] เทียบกับของเดิม — realCP ตอบ true ไม่แยก weak/strong (นี่คือรูรั่ว)");
// ⚠️ ใช้ชื่อที่ยังไม่ได้อยู่ในลิสต์ชื่อลวง — เอ็มซีพีไอ ถูกย้ายไปตัดทิ้งตั้งแต่ด่านแรกแล้ว
// (ถ้าใช้เคสที่ถูกตัดแล้ว realCP จะตอบ false ด้วย ข้อนี้จะกลายเป็นวัดไม่โดน)
ok("realCP มองว่าเอสซีพีเค = ข่าวเครือ", realCP("บริษัทเอสซีพีเค จำกัด") === true);
ok("cpEvidence แยกออก", cpEvidence("บริษัทเอสซีพีเค จำกัด") === "weak");
...
console.log("\n[11] 💊 ยา vs #️⃣ แฮชแท็ก — 2 ใบที่เจ้าของส่งมา 9 ก.ย. 2026");
{
  // ทั้งคู่เคยได้ "weak" -> ส่งให้ AI -> AI ตอบผิดทั้งคู่ (ตัดใบที่ควรเก็บ · เก็บใบที่ควรตัด)
  const KEEP = "CPทุกคำใส่ใจFilmAwards2026 - เข้มข้น กับคนข่าวเกษตรตัวจริง";
  const CUT = "รู้จัก 'Xanax' ยาคลายกังวลกลุ่มเบนโซไดอะซีพีน ทำไมต้องควบคุมการใช้ - Matichon";
  ok("🎯 แฮชแท็กของเครือ (CP ติดอักษรไทย) = strong", cpEvidence(KEEP) === "strong", cpEvidence(KEEP) || "(ว่าง)");
  ok("🎯 ชื่อกลุ่มยา เบนโซไดอะซีพีน = ไม่ใช่เครือ", cpEvidence(CUT) === "", cpEvidence(CUT));

  ok("ชื่อยาภาษาอังกฤษก็ไม่ใช่", cpEvidence("benzodiazepine addiction on the rise") === "");
  ok("แฮชแท็กแบบอื่นก็เข้า", cpEvidence("CPเพื่อชุมชน มอบทุนการศึกษา") === "strong");

  // 🚫 ต้องไม่จับเกิน — มีอักษรละตินนำหน้า CP ห้ามนับเป็นเครือ
  ok("🚫 MCPไทยแลนด์ ไม่ใช่เครือ", cpEvidence("MCPไทยแลนด์ เปิดตัวสินค้าใหม่") === "",
    cpEvidence("MCPไทยแลนด์ เปิดตัวสินค้าใหม่"));
  // 🚫 มีช่องว่างคั่น = ยังเป็น weak ตามเดิม ไม่ใช่ strong ฟรี
  ok("🚫 'CP ทุกคำ' (มีช่องว่าง) ยังต้องให้ AI ตัดสิน",
    cpEvidence("ผู้ป่วย CP ต้องการการดูแล") === "weak", cpEvidence("ผู้ป่วย CP ต้องการการดูแล"));
  // 🚫 ชื่อจริงที่มีคำใกล้เคียงต้องไม่พัง
  ok("🚫 ซีพีพีซี (CPPC) ยังเป็นบริษัทในเครือ", cpEvidence("ซีพีพีซี ลงทุนเพิ่ม") === "strong",
    cpEvidence("ซีพีพีซี ลงทุนเพิ่ม"));
}
