...
console.log("\n[2b] เปิดจากในแอปแชต — ติดตั้งไม่ได้ ต้องบอกให้ออกไปเบราว์เซอร์ก่อน");
{
  ...
  for (const [label, ua] of [["iPhone", LINE_IOS], ["Android", LINE_AND]]) {
    ...
    // ⚠️ เจ้าของสั่งเปลี่ยนคำ 25 ส.ค. 2026 — พูดจากมุมของสิ่งที่ผู้ใช้อยากได้
    //    ไม่ใช่ขึ้นต้นด้วย "ติดตั้งจากในไลน์ไม่ได้" ซึ่งอ่านแล้วเหมือนเว็บมีปัญหา
    ok(`ไลน์/${label} พูดจากมุมที่ผู้ใช้อยากได้`, /ถ้าต้องการจะติดตั้งเป็น app/.test(txt), txt.slice(0, 90));
    ok(`ไลน์/${label} 🚫 ไม่ขึ้นต้นแบบบอกว่าทำไม่ได้`, !/ไม่ได้ —/.test(txt), txt.slice(0, 90));
    // วิธีต้องตรงกับแอปนั้นจริงๆ — ไลน์ปุ่มอยู่มุมขวาล่าง เฟซบุ๊กอยู่มุมขวาบน
    ok(`ไลน์/${label} บอกตำแหน่งของไลน์ (มุมขวาล่าง)`, /มุมขวาล่าง/.test(txt), txt.slice(0, 90));
