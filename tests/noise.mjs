...
console.log("\n════════ 🏷 หน้ารวมป้ายกำกับ/หมวดหมู่ ไม่ใช่ข่าว (11 ก.ย. 2026) ════════");
{
  // เจ้าของส่งภาพมา: "ป้ายกำกับ: ซีพี - ไทยโพสต์" — หน้า tag ของเว็บ ไม่มีเนื้อข่าว
  // ⚠️ ด่านชื่อเครือปล่อยผ่าน เพราะชื่อเครืออยู่ในพาดหัวเต็มๆ (cpEvidence = strong)
  const { noiseReason: nr } = await load("../functions/api/trend/feeds.js");
  const w = (t, link = "https://x/1") =>
    nr({ title: t, link, snippet: "" }, t.toLowerCase(), "alert1");

  ok("🎯 เคสจริง: ป้ายกำกับ: ซีพี - ไทยโพสต์",
    w("ป้ายกำกับ: ซีพี - ไทยโพสต์", "https://www.thaipost.net/tag/cp/") === "archive-page",
    String(w("ป้ายกำกับ: ซีพี - ไทยโพสต์")));
  for (const t of [
    "Tag: CP All - Thai Post",
    "แท็ก: ปลาหมอคางดำ",
    "หมวดหมู่: PM 2.5 - เว็บข่าว",
    "Category: Sustainability",
  ]) ok(`รูปแบบอื่นของหน้าเดียวกัน: ${t}`, w(t) === "archive-page", String(w(t)));

  // 🚫 ต้องอยู่ต้นพาดหัว + มี : เท่านั้น ไม่งั้นกินข่าวจริง
  for (const t of [
    "สั่งติดป้ายกำกับสินค้าให้ครบก่อนวางขาย ซีพีเอฟ ยืนยันพร้อม",
    "ซีพี ออลล์ แจ้งผลประกอบการไตรมาส 2 กำไรโต",
    "เปิดหมวดหมู่สินค้าใหม่ในเซเว่น อีเลฟเว่น",
  ]) ok(`🚫 ห้ามตัดข่าวจริง: ${t.slice(0, 34)}`, w(t) === null, String(w(t)));
}

console.log("\n════════ 🚚 CP Foton = ผู้นำเข้ารถ ไม่ใช่เครือ (11 ก.ย. 2026) ════════");
{
  const { cpEvidence } = await load("../functions/api/_lib/noise.js");

  ok("🎯 เคสจริง: CP Foton จับมือ SPARK EV … = ไม่ใช่เครือ",
    cpEvidence("CP Foton จับมือ SPARK EV ลุยเพิ่มสถานีชาร์จกว่า 50 แห่ง - ผู้จัดการออนไลน์") === "",
    cpEvidence("CP Foton จับมือ SPARK EV ลุยเพิ่มสถานีชาร์จกว่า 50 แห่ง"));
  ok("ชื่อไทยก็ต้องไม่ใช่",
    cpEvidence("บริษัท ซีพี โฟตอน เซลส์ จำกัด เปิดตัวรถใหม่") === "",
    cpEvidence("บริษัท ซีพี โฟตอน เซลส์ จำกัด เปิดตัวรถใหม่"));
  ok("เขียนติดกันก็ต้องจับได้", cpEvidence("CPFoton ส่งมอบรถ") === "");

  // 🚫 ห้ามกินชื่อจริงของเครือ
  for (const t of [
    "ซีพี ออลล์ แจ้งผลประกอบการ",
    "ซีพีเอฟ เปิดโรงงานใหม่",
    "ซีพี แอ็กซ์ตร้า ขยายสาขา",
  ]) ok(`🚫 ยังต้องเป็นเครือ: ${t}`, cpEvidence(t) === "strong", cpEvidence(t));
}

console.log("\n════════ 🏗⚽ เอ็มซีพี เครน + ทรูแบงค็อก (14 ก.ย. 2026) ════════");
{
  const { cpEvidence } = await load("../functions/api/_lib/noise.js");
  const { noiseReason: nr } = await load("../functions/api/trend/feeds.js");
  const w = (t, link = "https://x/1") =>
  ...
  // 🏗 "เอ็มซีพี" — ด่าน weak จับไม่ได้ เพราะข้างหลังเป็นช่องว่าง จึงได้ strong = ผ่านฟรี
  ok("🎯 เคสจริง: บจก.เอ็มซีพี เครน = ไม่ใช่เครือ",
    cpEvidence("คุณยุ บจก.เอ็มซีพี เครน - Truck2Hand.com") === "",
    cpEvidence("คุณยุ บจก.เอ็มซีพี เครน - Truck2Hand.com"));
  ok("🎯 เคสจริง: ตัดที่โดเมนเว็บขายรถด้วย",
    w("คุณยุ บจก.เอ็มซีพี เครน", "https://www.truck2hand.com/p/1") === "shopping",
    String(w("คุณยุ บจก.เอ็มซีพี เครน", "https://www.truck2hand.com/p/1")));
  for (const h of ["one2car.com", "taladrod.com", "chobrod.com"])
    ok(`ตัดโดเมนเว็บขายรถ: ${h}`, w("ซีพี ประกาศขายรถ", `https://www.${h}/p/1`) === "shopping");

  // ⚽ ทรู แบงค็อก ยูไนเต็ด = สโมสรฟุตบอล
  ok("🎯 เคสจริง: ช้างศึกฯ อุ่นเครื่องกับทรูแบงค็อก ยู21 = ไม่ใช่เครือ",
    cpEvidence("ช้างศึกทีมชาติไทย อุ่นเครื่องกับทรูแบงค็อก ยู21 พร้อมบู๊อชก. นัดแรก 16 ก.ย.นี้") === "",
    cpEvidence("ช้างศึกทีมชาติไทย อุ่นเครื่องกับทรูแบงค็อก ยู21"));
  ok("ชื่ออังกฤษก็ต้องไม่ใช่", cpEvidence("True Bangkok United เปิดตัวนักเตะใหม่") === "");
  ok("เขียนเว้นวรรคก็ต้องจับได้", cpEvidence("ทรู แบงค็อก ยูไนเต็ด ชนะ 2-0") === "");

  // 🚫 ฝั่งนี้สำคัญกว่า — ตัดแค่ชื่อสโมสร/ชื่อบริษัทคนละเจ้า ห้ามลามไปกินข่าวจริงของเครือ
  for (const t of [
    "ทรู คอร์ปอเรชั่น ประกาศผลประกอบการไตรมาส 2",
    "ทรูมูฟ เอช เปิดตัวแพ็กเกจใหม่",
    "ทรูดิจิทัล กรุ๊ป เปิดตัวบริการใหม่",
    "ซีพี ออลล์ สนับสนุนทีมฟุตบอลเยาวชนไทย",
    "ซีพีเอฟ แจ้งผลประกอบการไตรมาส 2",
    "ซีพี แอ็กซ์ตร้า เปิดสาขาใหม่",
  ]) ok(`🚫 ยังต้องเป็นเครือ: ${t.slice(0, 32)}`, cpEvidence(t) === "strong", cpEvidence(t));
}
