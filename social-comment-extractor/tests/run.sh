...
# ── 0. 🚫 ห้ามมีที่อยู่ไฟล์ของ "เครื่องที่รัน session" ฝังอยู่ในเทสต์ ──────
#
# 🐞 เจอจริง 11 ก.ย. 2026: lensconsistency.mjs เขียน
#      readFileSync("/home/user/cp_dashboard/functions/.../_core.js")
#    → รันที่นี่ผ่านสบาย แต่ **ที่ GitHub พังทันที** (ENOENT) เพราะไม่มีโฟลเดอร์นั้น
#    เป็นตระกูลเดียวกับที่อยู่เบราว์เซอร์ที่ย้ายไป browser.cjs ไปแล้ว — ไล่ไม่หมดรอบแรก
#
# ด่านนี้จับตั้งแต่ก่อนรัน ไม่ต้องรอไปตกที่ GitHub แล้วค่อยรู้
bad="$(grep -ln '"/home/\|'"'"'/home/' "$HERE"/*.mjs "$HERE"/*.cjs 2>/dev/null || true)"
if [ -n "$bad" ]; then
  echo "❌ มีที่อยู่ไฟล์แบบตายตัว (/home/…) ในเทสต์ — เครื่องอื่นรันไม่ได้:"
  echo "$bad" | sed 's/^/   /'
  echo "   ✅ ใช้ import.meta.url / __dirname หาที่อยู่เอาเอง"
  exit 1
fi
